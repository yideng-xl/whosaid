    ...
    return recordingClose.run(async () => {
      ...
      try {
        await runRecordingCloseFlow(recordingSnapshot, {
          stopAndSave: async () => { await finalizeRecordingOnce(); },
          persistFinalPath: persistFinalPathForClose,
