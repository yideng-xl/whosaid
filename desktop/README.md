...
- 停止后先把双路音轨（或仅系统音轨）合成为 `.m4a`，展示播放器；可以编辑录音名称、继续录制多段，再逐条确认是否创建分人和转写任务。
- 双轨混音以系统声为参考，对麦克风中的相关回灌做自适应消除，降低腾讯会议与扬声器同时使用时的回音；声学环境差异较大，仍建议优先使用耳机。

录音最终文件位于
`~/Library/Application Support/whosaid/recordings/`。录制或合成未完成时，原始音轨和
会话清单保留在其 `.incomplete/<会话 UUID>/` 子目录；应用下次启动会显示未完成录音，
可选择“结束并保存”，或先保存旧段再开始新的一段。旧音轨不会被原地追加覆盖。
...
本机更新必须使用固定代码签名身份，避免 macOS 把每次构建识别成新应用并重置录音权限：

```bash
npm run tauri:build:signed -- --bundles app
```

脚本默认要求登录钥匙串中存在 `whosaid Local Development`。身份缺失时会直接失败，禁止
自动退回临时签名。正式对外发布时应通过 `APPLE_SIGNING_IDENTITY` 使用 Developer ID。
